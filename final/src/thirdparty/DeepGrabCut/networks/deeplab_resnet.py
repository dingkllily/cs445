...
from thirdparty.DeepGrabCut.mypath import Path
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, dilation_=1, downsample=None):
        ...
        self.conv2 = nn.Conv2d(
            planes, planes, kernel_size=3, stride=1, padding=padding, bias=False, dilation=dilation_  # change
        )
        ...
        for module, module_ori in zip(self.modules(), base_network.Scale.modules()):
            if isinstance(module, nn.Conv2d) and isinstance(module_ori, nn.Conv2d):
                ...
                if not flag and nInputChannels != 3:
                    ...
                    print(
                        "Skipping Conv layer with size: {} and target size: {}".format(
                            module.weight.data.shape, module_ori.weight.data.shape
                        )
                    )
            elif (
                isinstance(module, nn.BatchNorm2d)
                and isinstance(module_ori, nn.BatchNorm2d)
...
def resnet101(
    n_classes, pretrained=False, nInputChannels=3, classifier="atrous", dilations=(2, 4), strides=(2, 2, 2, 1, 1)
):
...
class MS_Deeplab(nn.Module):
    ...
    def forward(self, x):
        ...
        self.interp1 = nn.Upsample(
            size=(int(input_size * 0.75) + 1, int(input_size * 0.75) + 1), mode="bilinear", align_corners=True
        )
        self.interp2 = nn.Upsample(
            size=(int(input_size * 0.5) + 1, int(input_size * 0.5) + 1), mode="bilinear", align_corners=True
        )
...
def Res_Deeplab(n_classes=21, pretrained=False):
    ...
    if pretrained:
        ...
        saved_state_dict = torch.load(
            os.path.join(Path.models_dir(), pth_model), map_location=lambda storage, loc: storage
        )
        if n_classes != 21:
